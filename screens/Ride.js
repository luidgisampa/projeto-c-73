...
export default class RideScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    if (!transactionType) {
      ...
      console.log("Por favor insira/digitalize um ID válido");
    ...
    if (bikeRef.docs.length == 0) {
      ...
      bikeRef.docs.map(doc => {
        if (!doc.data().under_maintenance) {
          ...
          console.log(doc.data().maintenance_message)
        }
      });
    }
    ...
    if (userRef.docs.length == 0) {
      ...
      userRef.docs.map(doc => {
        if (!doc.data().bike_assigned) {
          ...
          console.log("Finalize o passeio atual para alugar outra bicicleta");
          this.setState({
            bikeId: "",
            userId: ""
          });
  ...
  checkUserEligibilityForEndRide = async (bikeId, userId) => {
    ...
    transactionRef.docs.map(doc => {
      ...
      if (lastBikeTransaction.user_id === userId) {
        ...
        console.log("Essa bicicleta está alugada por outro usuário")
        this.setState({
          bikeId: ""
        });
      }
